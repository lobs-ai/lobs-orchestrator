...
class WorkerManager:
    ...
    def spawn_worker(self, task: Dict[str, Any], project_id: str, agent_type: str = "task-runner"):
        task_id = task["id"]
        ...
        try:
            ...
            subprocess.run(["git", "pull", "--rebase"], cwd=workspace, check=True, capture_output=True)
        except subprocess.CalledProcessError as e:
            logger.error(f"Failed to sync project repo {project_id}: {e.stderr.decode()}")
            return None
        ...
        try:
            ...
            logger.info(f"Spawned worker {process.pid} for task {task_id} on {project_id} using agent {agent_type}")
            return process
    ...
    def check_workers(self):
        ...
        for task_id, (process, project_id) in self.active_workers.items():
            ...
            if retcode is not None:
                # Process finished
                logger.info(f"Worker process for task {task_id} on {project_id} finished with code {retcode}")
    ...
    def finalize_project_changes(self, task_id: str, project_id: str) -> bool:
        ...
        try:
            ...
            status = subprocess.run(["git", "status", "--porcelain"], cwd=project_path, check=True, capture_output=True).stdout.decode()
            if not status:
    ...
    def update_active_worker_status(self):
        ControlManager.request_op({
            ...
            "updates": {
                ...
                "currentTask": None if not self.active_workers else list(self.active_workers.keys())[0],
                "lastHeartbeat": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
            }
        })
